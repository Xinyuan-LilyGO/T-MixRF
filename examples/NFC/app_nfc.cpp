...
{
    ReturnCode res = false;
    ndef_notify_cb = notify_cb;
    ndef_event_cb = event_cb;
    Serial.print("Initializing NFC: ");
    res = NFCReader.rfalNfcInitialize();
    if (!(res == ST_ERR_NONE)) {
        Serial.printf("Failed to find NFC Reader - check your wiring! code=%d\n", res);
        return false;
    } else {
        Serial.printf("Initializing NFC Reader succeeded, code=%d\n", res);
    }
